    ...
    def _loop(self):
        ...
        while self.running:
            ...
            try:
                ...
                if frame is not None and not is_frame_too_dark(frame):
                    count, boxes = detect_people(frame)
                    now = datetime.now(TZ)
                    insert_detection(self.db_conn, self.camera_id, count, now)
                    # Store latest frame + boxes for snapshot API
                    ...
                    # Save annotated snapshot when people detected
                    if count > 0:
                        save_detection_snapshot(self.camera_id, frame, boxes, count, now)
                    logger.info("[%s] Detected %d people", self.camera_id, count)
                else:
                    logger.warning("[%s] Frame capture failed — stream may be down", self.camera_id)
            except Exception:
